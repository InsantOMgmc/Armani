...
if (basket && closeBasket && closeBasket) {
...
const countInputs = document.querySelectorAll('.cart-info__count-input');
const addButtons = document.querySelectorAll('.cart-info__count-add');
const removeButtons = document.querySelectorAll('.cart-info__count-remove');

addButtons.forEach((button, index) => {
    button.addEventListener('click', () => {
        countInputs[index].value = parseInt(countInputs[index].value) + 1;
    });
});

removeButtons.forEach((button, index) => {
    button.addEventListener('click', () => {
        if (parseInt(countInputs[index].value) > 1) {
            countInputs[index].value = parseInt(countInputs[index].value) - 1;
        }
    });
});
