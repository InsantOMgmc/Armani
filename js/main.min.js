...
if (basket) {
    openButtons.forEach(button => {
        button.addEventListener("click", () => {
            ...
            toggleButtonsDisplay(openButtons, false);

            // Получаем текущее положение скролла
            const scrollTop = window.scrollY || document.documentElement.scrollTop;

            // Если скролл ниже определенной точки, меняем позицию корзины
            if (scrollTop > 150) {
                basket.style.position = 'absolute';  // Корзина будет привязана к текущей позиции
                basket.style.top = `${scrollTop}px`;  // Смещаем корзину вниз относительно скролла
            } else {
                basket.style.position = 'absolute';  // По умолчанию корзина на 150px от верха
                basket.style.top = '150px';
            }
        });
    });
    ...
    closeButtons.forEach(button => {
        button.addEventListener("click", () => {
            ...
            toggleButtonsDisplay(openButtons, true);

            // Сбрасываем позицию корзины при закрытии
...
function toggleButtonsDisplay(buttons, show) {
    buttons.forEach(button => {
        // Избегаем изменения стиля кнопки с классом 'cart-mobile'
        if (!button.classList.contains('cart-mobile')) {
